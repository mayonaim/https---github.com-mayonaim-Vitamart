...
class NotificationTab extends GetView<HomeController> {
  NotificationTab({super.key});
  @override
  Widget build(BuildContext context) {
    return ListView(
      children: [
        VAppBar(title: 'Notifikasi', includeBackButton: false),
  ...
  Widget _buildNotificationsList(theme) {
    return Obx(() {
      final itemsList = controller.transactions;
  ...
  Widget _buildNotification(TransactionModel item, theme) {
    return Padding(
      ...
      child: GestureDetector(
        onTap: () {
          Get.toNamed('/transaction/${item.id}');
        },
        child: Container(
          height: 120,
